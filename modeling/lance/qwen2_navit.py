...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    # transformers' PreTrainedModel.device is a read-only property; comfy's ModelPatcher
    # assigns model.device for placement bookkeeping, so shadow it with a settable one.
    @property
    def device(self):
        if getattr(self, "_comfy_device", None) is not None:
